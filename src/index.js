...
function ajax(options = {}){
  ...
  for (let key in DEFAULTS) if (settings[key] === undefined) settings[key] = DEFAULTS[key]

  if (!settings.crossDomain) settings.crossDomain = /^([\w-]+:)?\/\/([^\/]+)/.test(settings.url) && RegExp.$2 != window.location.host
  if (isJsonP(settings)) return sendJsonP(settings)
...
function serialize(params, obj, scope){
  ...
  for (let key in obj) {
    ...
    if (!scope && array) add(value.name, value.value)
